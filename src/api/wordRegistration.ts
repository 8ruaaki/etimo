...
export const checkEtymologyMatch = async (
  ...
  if (!GEMINI_API_KEY) {
    ...
    return {
      ...
      parts: matched ? [
        { part: "ex", meaning: "外へ", relatedWords: [{ word: "exit", meaning: "出口", breakdown: "ex(外へ)＋it(行く)" }, { word: "export", meaning: "輸出する", breakdown: "ex(外へ)＋port(運ぶ)" }] },
        { part: "port", meaning: "運ぶ", relatedWords: [{ word: "portable", meaning: "持ち運び可能な", breakdown: "port(運ぶ)＋able(できる)" }, { word: "import", meaning: "輸入する", breakdown: "im(中へ)＋port(運ぶ)" }] }
      ] : []
    };
  }
...
【ステップ3: 語源分解（matched=true の場合のみ実行）】
matched=true の場合のみ、対象単語を「すべての語源パーツ（接頭辞、語根、接尾辞など）」に完全に分解してください。
※ユーザーの語源リストに含まれていないパーツであっても、単語を構成するすべてのパーツを漏れなく抽出して \`parts\` 配列に含めてください。（例：expire の場合、ex だけでなく spir/pire などすべての構成要素を抽出する）
そして、抽出したすべてのパーツそれぞれについて、意味と、同じ語源を持つ別の「非常に簡単な（できれば中学・高校初級レベルの）」英単語（対象単語以外）を【必ず3つずつ】提示してください。簡単な単語が見つからない場合は、少し難しい単語であっても構いませんので、必ず3つひねり出してください。
さらに、提示した関連語それぞれについて、その単語がどのように語源分解できるかを \`breakdown\` に必ず記載してください。（例：「ex(外)＋pire(息をする/魂)」「ex(外へ)＋port(運ぶ)」）

さらに、分解したそれぞれのパーツの意味を統合して、どのように現在の単語の意味につながっているのかを表す短い統合フレーズを \`integratedMeaning\` として出力してください。
その際、意味の変遷の各段階を必ず「→」記号でつないでください。
【重要】統合フレーズの最初は、必ず「それぞれの語源パーツの意味を『＋』で結んだもの」にし、最後は必ず最終的な意味である「対象単語の意味」で終わるようにしてください。
例：「外へ＋運ぶ→○○→輸出する」「完全に＋通る＋できる→○○→通り抜けることができない」

※ matched=false または isRealWord=false の場合は、parts は空配列 []、integratedMeaning は空文字 "" にしてください。

重要：
出力における意味や関連語の意味などの説明文は、すべて必ず日本語のみで記述してください。英語の文章やフレーズを含めないでください。
...
`.trim();
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseSchema: {
            ...
            required: ["isRealWord", "matched", "targetWordMeaning", "integratedMeaning", "parts"]
          }
        },
        safetySettings: [
          {
    ...
    if (cleanText && !cleanText.endsWith('}')) {
      console.warn('[EtymologyCheck] JSONが途中で切れている可能性があります。自動修復を試みます。');
      ...
      let parsed: EtymologyCheckResult | null = null;
      for (const suffix of tryAppend) {
        try {
          parsed = JSON.parse(fixedText + suffix) as EtymologyCheckResult;
          break;
        } catch (e) {
          // ignore
        }
      ...
      if (parsed) {
        console.info('[EtymologyCheck] 途切れたJSONの修復に成功しました。');
        return parsed;
      } else {
        console.error('[EtymologyCheck] 途切れたJSONの修復に失敗しました。');
...
/**
 * Screen Bでのパーツ意味と統合ステップの自動生成 (Gemini API)
 */
export const autoCompleteIntegration = async (
  word: string,
  targetWordMeaning: string,
  parts: string[],
  currentPartMeanings: string[]
): Promise<{ partMeanings: string[]; intermediateSteps: string[] }> => {
  if (!GEMINI_API_KEY) {
    return {
      partMeanings: parts.map((p, i) => currentPartMeanings[i] || `モック意味(${p})`),
      intermediateSteps: ["モック変化1", "モック変化2"]
    };
  }

  const prompt = `
あなたは英語の語源・暗記法の専門家です。
ユーザーが英単語「${word}」（意味：「${targetWordMeaning}」）を以下のパーツに分割しました。
パーツ: ${parts.join(', ')}
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseSchema: {
            ...
            required: ["partMeanings", "intermediateSteps"]
          }
        },
        safetySettings: [
          { category: "HARM_CATEGORY_HARASSMENT", threshold: "BLOCK_NONE" },
