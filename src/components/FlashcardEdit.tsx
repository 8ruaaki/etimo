...
import { getFlashcard, updateFlashcard, renameFlashcard, type FlashcardItem as APIFlashcardItem } from '../api/flashcard';
...
export const FlashcardEdit: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFlashcard = async () => {
      ...
      try {
        ...
        const result = await getFlashcard(email, decodedTitle);
        if (result.success) {
          setCurrentTitle(decodedTitle);
          const flashcardsWithId = result.flashcards.map((card: any, index: number) => ({
            ...card,
            id: index + 1,
          }));
  ...
  const updateFlashcardField = (id: number, field: keyof FlashcardItem, value: any) => {
  ...
  const generateMeaning = async (id: number, word: string) => {
    ...
    updateFlashcardField(id, 'isGenerating', true);
    setError('');

    try {
      const result = await getWordMeaning(word);
      if (result.success && result.meaning) {
        updateFlashcardField(id, 'meaning', result.meaning);
  ...
  const generateEtymologyContent = async (id: number, word: string, type: 'true' | 'false' | 'katakana' | 'god') => {
    ...
    updateFlashcardField(id, 'isGeneratingEtymology', true);
    setError('');

    try {
      const result = await getEtymology(word, type, hint);
      if (result.success && result.etymology) {
        updateFlashcardField(id, 'etymology', result.etymology);
      } else if (result.error) {
        console.error(result.error);
        setError(result.error);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // タイトルが変更された場合は名前変更処理を実行
      if (currentTitle !== originalTitle) {
        const renameResult = await renameFlashcard(email, originalTitle, currentTitle);
        if (!renameResult.success) {
          setError(renameResult.error || '単語帳名の変更に失敗しました。');
          setSaving(false);
          return;
        }
      }
      ...
      // idフィールドを除外して送信
      const flashcardsToSave: APIFlashcardItem[] = flashcards.map(({ word, etymology, meaning }) => ({
        word,
        etymology,
        meaning,
      }));

      const result = await updateFlashcard(email, currentTitle, flashcardsToSave);

      if (result.success) {
        alert('単語帳が更新されました！');
        navigate('/flashcards');
      } else {
        setError(result.error || '単語帳の更新に失敗しました。');
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="glass-panel" style={{ width: '100%', maxWidth: '800px', padding: '40px' }}>
        <p style={{ color: 'var(--text-secondary)', textAlign: 'center' }}>読み込み中...</p>
      </div>
    );
  }
    ...
    <div className="glass-panel" style={{ width: '100%', maxWidth: '800px', padding: '40px', position: 'relative', overflow: 'visible' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="flex-between mb-8" style={{ alignItems: 'center' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 700 }}>単語帳編集</h1>
          <button 
            onClick={() => navigate('/flashcards')} 
            className="text-link" 
            style={{ 
            ...
            >
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input
                    ...
                    onChange={(e) => updateFlashcardField(flashcard.id, 'word', e.target.value)}
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    updateFlashcardField(flashcard.id, 'etymology', e.target.value);
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    updateFlashcardField(flashcard.id, 'meaning', e.target.value);
                    e.target.style.height = 'auto';
                    e.target.style.height = `${e.target.scrollHeight}px`;
                  }}
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '16px',
            background: saving ? 'rgba(59, 130, 246, 0.5)' : 'linear-gradient(135deg, var(--accent-color), #0ea5e9)',
            border: 'none',
            borderRadius: '12px',
            color: 'white',
            fontSize: '1.1rem',
